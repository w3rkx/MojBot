...
client.on("messageCreate", async message=>{
    ...
    if(message.content === "!RedBullZSkrzydlami"){

        message.reply("Wygrales LB Gratulacje, Jezeli wygrales 2/3-gi raz to nie wygrasz");
...
client.on("messageCreate", async (message) => {
    ...
    if (!message.content.startsWith("!sayd ")) return;
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(text)
        .setFooter({
            text: `Wysłane przez ${message.author.tag}`,
